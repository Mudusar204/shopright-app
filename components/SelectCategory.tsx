...
const SelectCategoryComponent = ({ item, selectedItem, setSelectedItem }: any) => {
    ...
    return (
        <View>
            <TouchableOpacity
                ...
                onPress={() => setSelectedItem(selectedItem.includes(item) ? selectedItem.filter((i: any) => i !== item) : [...selectedItem, item])}
            >
                <Text
                    style={[
                        styles.buttonText,
...
const createStyles = (colorTheme: 'light' | 'dark', width: number) => StyleSheet.create({
    button: {
        paddingVertical: 20,
        borderRadius: 12,
        borderWidth: 1,
